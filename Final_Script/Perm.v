...
module Perm(
    ...
    always@(posedge clk)begin
        if(rst)begin
            x0_t<=0;x1_t<=0;x2_t<=0;x3_t<=0;x4_t<=0;
            type<=0;
            rnd_no<=0;
            finish<=0;
            busy<=0;
            ...
            else if(busy==1&&rnd_no<type)begin
                x0_t<=x0;x1_t<=x1;x2_t<=x2;x3_t<=x3;x4_t<=x4;
                rnd_no<=rnd_no+1;
            end
            else if(busy==1&&rnd_no==type)begin
                S_out<={x4_t,x3_t,x2_t,x1_t,x0_t};
                finish<=1;
                busy<=0;
            end
        end
    ...
    sbox L2(
        .x0(x0_t),.x1(x1_t),.x2(a1),.x3(x3_t),.x4(x4_t),
